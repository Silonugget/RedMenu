...
namespace RedMenuClient.menus
{
    public class VehicleDistance
    {
        ...
        public VehicleDistance(int vehicle)
        {
            ...
            distance = GetDistanceBetweenCoords(pCoords.X, pCoords.Y, pCoords.Z, vCoords.X, vCoords.Y, true);
        }
    }
    ...
    public class VehicleMenu : BaseScript
    {
        ...
        private static void AddAddonVehicleSubmenu(Menu menu, string vehicleType, string name, string description)
        {
            ...
            // Add the menu item to the addon vehicle menu
            ...
            // Check if the vehicleConfigs dictionary has the vehicle type
            if (vehicleConfigs.ContainsKey(vehicleType))
            {
                var vehicles = vehicleConfigs[vehicleType]["vehicles"];

                // Loop through the vehicle list and add each to the submenu
                foreach (var vehicle in vehicles)
                {
                    string vehicleName = vehicle["name"].ToString();
                    MenuItem item = new MenuItem(vehicleName);
                ...
                submenu.OnItemSelect += async (m, item, index) =>
                {
                    ...
                    string selectedVehicleName = vehicles[index]["model"].ToString();
                    uint model = (uint)GetHashKey(selectedVehicleName);
                    ...
                    if (IsModelInCdimage(model))
                    {
                        ...
                        if (selectedVehicleName == "hotairballoon01")
                        {
            ...
            {
                Debug.WriteLine($"^1[ERROR] No vehicle data found for type: {vehicleType}.^7");
            }
        }
    ...
    AddVehicleSubmenu(regularVehiclesMenu, data.VehicleData.MiscHashes, "Misc", "Spawn a miscellaneous vehicle.");
                ...
                // Dynamically add submenus based on vehicle types in the vehicleConfigs
                foreach (var vehicleType in vehicleConfigs.Keys)
                {
                    string typeName = char.ToUpper(vehicleType[0]) + vehicleType.Substring(1); // Capitalize the first letter
                    AddAddonVehicleSubmenu(addonVehiclesMenu, vehicleType, typeName, $"Spawn a {vehicleType}.");
                }
            }
